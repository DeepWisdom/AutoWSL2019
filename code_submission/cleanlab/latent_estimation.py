...
def compute_confident_joint(
    ...
    if thresholds is None:
        thresholds = [np.median(psx[:,k][s == k]) for k in range(K)] # P(s^=k|s=k)
        print(thresholds)
...
def estimate_confident_joint_and_cv_pred_proba(
    ...
    for seed in seed_list:
        ...
        for k, (cv_train_idx, cv_holdout_idx) in enumerate(kf.split(X, s)):
            ...
            psx[cv_holdout_idx] = (psx_cv + psx[cv_holdout_idx])/tick
        tick += 1
    # clf.fit(X, s)
    # psx = clf.predict_proba(X)
